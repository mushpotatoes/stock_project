...
def split_dataset(df, train_frac=0.85, val_frac=0.15, test_frac=None, random_state=42):
    ...
    df_shuffled = df.iloc[:].reset_index(drop=True)
    n = len(df_shuffled)
    ...
    # exit()
    if test_frac == None:
        test_df = df.iloc[first_row.name:].copy()
    else:
...
def evaluate_model(clf, df):
    ...
    # df['target'] = (df['future_slope'] > EVAL_RATIO).astype(int)
...
def compute_slopes_for_timeframe(timeframe, df):
    ...
    saves the scaler to a dedicated directory for later use, and returns the
    standardized values.
    """
    # print(f"Adding {timeframe} minute slopes (Takes a while)")
    # Create a local copy to work on
    ...
    # means = np.array([3e-07, 0])
    # stds = np.array([0.0001, 0.000025])
    # new_cols_df_standardized = (new_cols_df - means) / stds
    # exit()
    # print(f"Done with {timeframe} minute slopes")
    
    # Standardize the computed columns.
    ...
    # Save the scaler to a special directory (e.g., 'scalers').
    scalers_dir = "scalers"
    ...
    # print(new_cols_df_standardized)
...
def main(model_type="classifier", retrain=False, model_path="random_forest_model.pkl", save_trees_path=None):
    ...
    train_df, val_df, test_df = split_dataset(other_df, train_frac=0.85, val_frac=0.15, test_frac=None, random_state=42)
    if test_df is None:
        test_df_len = 0
    else:
